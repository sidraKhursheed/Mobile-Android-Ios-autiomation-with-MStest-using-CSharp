...
namespace Ios_Android_Project.PageObjectModel
{
    public class BaseClass
    {
       ...
       // public AndroidDriver<AppiumWebElement> androiddriver;
        public TimeSpan ts = TimeSpan.FromSeconds(5);
        ...
        public void TestClean()
        {
            ...
            mobiledriver.RemoveApp("com.wggesucht.android");
        ...
        {
            Base.contextSwitching();
            Thread.Sleep(ts);

            IWebElement username = mobiledriver.FindElement(By.CssSelector("#username_id"));
            Console.WriteLine("Username found");
            username.Click();
            Thread.Sleep(ts);
            username.SendKeys(TestContext.Properties["UserName"].ToString());
            Console.WriteLine("Enter Username");
            Thread.Sleep(ts);
            IWebElement password = mobiledriver.FindElement(By.CssSelector("#password_id"));
            password.Click();
            password.SendKeys(TestContext.Properties["Password"].ToString());
            Console.WriteLine("Enter Password");
            Thread.Sleep(ts);
            mobiledriver.HideKeyboard();
            Thread.Sleep(ts);
            IWebElement Authenticate = mobiledriver.FindElement(By.CssSelector("#authenticateBtn"));
            Authenticate.Click();
            Console.WriteLine("Login sucessfull");
            Thread.Sleep(50000);
            mobiledriver.SwitchTo().Frame("mainIframe");
        }
        ...
        private void StartAppAndroid()
        {
            ...
            var packagePath = @"C:\Users\HP PROBOOK\Downloads\WG_Gesucht.apk";
            ...
            capabilities.AddAdditionalCapability(AndroidMobileCapabilityType.AppPackage, "com.wggesucht.android");
